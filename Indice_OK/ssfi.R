...
##  Script to calculate SFI                                                     ##
##  Pierre L'HERMITE - 2017-10-16 - ISD.R                                       ##
##______________________________________________________________________________##
...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  ...
  pdf(paste0(DIR_GRAPHE,"SSFI_BV/SSFI_", Delta, ".pdf"), paper ="a4r", height=0, width=0)
  for (iBV in c(1:length(infobv$code))){
    ...
    #Fonction pour calculer le SSFI
  ...
  save(Sech, file = paste0(DIR_GRAPHE, "SSFI_BV/Sech_SSFI_", Resultat$Delta,".RData"))
  dev.off()
}
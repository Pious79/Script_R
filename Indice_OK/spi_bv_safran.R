...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  ...
  pdf(paste0(DIR_GRAPHE,"SPI_BV_SAFRAN/SPI_BV_Pluie_Safran_", Delta, ".pdf"), paper ="a4r", height=0, width=0)
  for (iBV in c(1:length(infobv$code))){
    ...
    Pday <- zoo(BasinData$TabObsPsafran , order.by = as.Date(BasinData$TabDatesR))
  ...
  save(Sech, file = paste0(DIR_GRAPHE, "SPI_BV_SAFRAN/Sech_SPI_BV_Pluie_SAFRAN_", Resultat$Delta, ".RData"))
  dev.off()
}

...
##  Pierre L'HERMITE - 2018-01-11 - Plot_versus_ISSPvsPNI.R                   ##
##____________________________________________________________________________##
...
pdf(paste0(DIR_GRAPHE,"Versus_ISSPvsPNI.pdf"), paper ="a4r", height=0, width=0)
layout(matrix(c(1, 2,
...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  for (iBV in c(12, 14, 3, 5)){
    ...
    Corre <- cor(CorISSP, CorPNI, method = "pearson")
    
    #Plot
    plot(ResISSP$ISSP, ResPNI$PNI, axes=FALSE, panel.first = grid(), pch = 18,
         ylim = c(0, 200), xlim = c(-4, 4), col = "grey34", 
         xlab = "ISSP", ylab = "PNI", main = paste(PluvioData$PluvioName, Delta))

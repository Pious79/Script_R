...
##  Pierre L'HERMITE - 2017-11-06 - Plot_versus_RAIvsSPI.R                    ##
##____________________________________________________________________________##
...
pdf(paste0(DIR_GRAPHE,"Versus_RAIvsSPI.pdf"), paper ="a4r", height=0, width=0)
layout(matrix(c(1, 2,
...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  for (iBV in c(12, 14, 3, 5)){
    ...
    Corre <- cor(CorSPI, CorRAI, method = "pearson")
    
    #Plot SPI
    plot(ResSPI$SPI, ResRAI$RAI, axes=FALSE, panel.first = grid(), pch = 18,
         ylim = c(-4, 4), xlim = c(-4, 4), col = "grey34", 
         xlab = "SPI", ylab = "RAI", main = paste(PluvioData$PluvioName, Delta))

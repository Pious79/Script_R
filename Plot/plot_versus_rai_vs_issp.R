...
##  Script to plot indices ISP vs RAI                                         ##
##  Pierre L'HERMITE - 2017-11-06 - Plot_versus_RAIvsISSP.R                   ##
##____________________________________________________________________________##
...
pdf(paste0(DIR_GRAPHE,"Versus_RAIvsISSP.pdf"), paper ="a4r", height=0, width=0)
layout(matrix(c(1, 2,
...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  for (iBV in c(12, 14, 3, 5)){
    ...
    CorRAI <- ResRAI$RAI
    CorRAI[is.na(ResRAI$RAI)] <- 0
    
    Corre <- cor(CorISSP, CorRAI, method = "pearson")
    
    #Plot
    plot(ResISSP$ISSP, ResRAI$RAI, axes=FALSE, panel.first = grid(), pch = 18,
         ylim = c(-4, 4), xlim = c(-4, 4), col = "grey34", 
         xlab = "ISSP", ylab = "RAI", main = paste(PluvioData$PluvioName, Delta))

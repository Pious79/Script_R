...
##  Pierre L'HERMITE - 2018-01-11 - Plot_versus_RAIvsPNI.R                   ##
##____________________________________________________________________________##
...
pdf(paste0(DIR_GRAPHE,"Versus_RAIvsPNI.pdf"), paper ="a4r", height=0, width=0)
layout(matrix(c(1, 2,
...
for(Delta in c(1, 3, 6, 9, 12, 24)){
  for (iBV in c(12, 14, 3, 5)){
    ...
    Corre <- cor(CorRAI, CorPNI, method = "pearson")
    
    #Plot
    plot(ResRAI$RAI, ResPNI$PNI, axes=FALSE, panel.first = grid(), pch = 18,
         ylim = c(0, 200), xlim = c(-4, 4), col = "grey34", 
         xlab = "RAI", ylab = "PNI", main = paste(PluvioData$PluvioName, Delta))
